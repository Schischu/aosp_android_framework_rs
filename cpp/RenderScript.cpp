...
#if defined(__LP64__) && defined(RS_COMPATIBILITY_LIB)
    // TODO: need to get the nativelibdir for 64bit compat lib in C++
    const char * nativeLibDir = nullptr;
    dispatchTab.SetNativeLibDir(mContext, nativeLibDir, length);
#endif
    if (mContext == 0) {
