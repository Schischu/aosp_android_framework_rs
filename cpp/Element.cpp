...
Element::Element(void *id, android::RSC::sp<RS> rs,
                 android::RSC::sp<Element> * elements,
                 size_t elementCount,
                 const char ** elementNames,
                 uint32_t * elementNameLens,
    ...
    mElements = (sp<Element> *)calloc(mElementsCount, sizeof(sp<Element>));
    mElementNames = (char **)calloc(mElementsCount, sizeof(char *));
    mElementNameLens = (uint32_t*)calloc(mElementsCount, sizeof(uint32_t));
    mArraySizes = (uint32_t*)calloc(mElementsCount, sizeof(uint32_t));

    memcpy(mElements, elements, mElementsCount * sizeof(sp<Element>));
    memcpy(mArraySizes, arraySizes, mElementsCount * sizeof(uint32_t));

    //copy strings (char array)
    memcpy(mElementNameLens, elementNameLens, mElementsCount * sizeof(uint32_t));
    for (size_t ct = 0; ct < mElementsCount; ct++ ) {
        uint32_t elemNameLen = mElementNameLens[ct];
...
Element::Builder::Builder(android::RSC::sp<RS> rs) {
    ...
    mElementsCount = 0;
    // Initialize space
    mElements = (sp<Element> *)calloc(mElementsVecSize, sizeof(sp<Element>));
    mElementNames = (char **)calloc(mElementsVecSize, sizeof(char *));
    mElementNameLens = (uint32_t*)calloc(mElementsVecSize, sizeof(uint32_t));
...
void Element::Builder::add(android::RSC::sp</*const*/ Element>e, const char * name, uint32_t nameLen, uint32_t arraySize) {
    ...
    if (mElementsCount >= mElementsVecSize) {
        ...
        sp<Element> * newElements = (sp<Element> *)calloc(mElementsVecSize, sizeof(sp<Element>));
        char ** newElementNames = (char **)calloc(mElementsVecSize, sizeof(char *));
        uint32_t * newElementNameLens = (uint32_t*)calloc(mElementsVecSize, sizeof(uint32_t));
        uint32_t * newArraySizes = (uint32_t*)calloc(mElementsVecSize, sizeof(uint32_t));        

        memcpy(newElements, mElements, mElementsCount * sizeof(sp<Element>));
        memcpy(newElementNames, mElementNames, mElementsCount * sizeof(char *));
        memcpy(newElementNameLens, mElementNameLens, mElementsCount * sizeof(uint32_t));
    ...
    mElementNameLens[mElementsCount] = nameLen;
    mElementNames[mElementsCount] = (char *)calloc(mElementsVecSize, sizeof(char));
...
android::RSC::sp<const Element> Element::Builder::create() {
    ...
    void *id = RS::dispatch->ElementCreate2(mRS->getContext(),
                                ...
                                (const char **)mElementNames, fieldCount * sizeof(size_t),
                                mArraySizes, (const uint32_t *)&mArraySizes[0], fieldCount);

    free(elementArray);
    return new Element(id, mRS, mElements, mElementsCount, (const char **)mElementNames, mElementNameLens, mArraySizes);
}
