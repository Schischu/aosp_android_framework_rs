...
// Launch a kernel.
// The execution of the kernel is controlled by the callback function.
void RsdCpuReferenceImpl::launchThreads(WorkerCallback_t cbk, void *data) {
    ...
    if (mtls && mtls->dim.y <= 1 && mtls->end.x <= mtls->start.x + mtls->mSliceSize) {
...
// Inputs:
//   mtls - The MTLaunchStruct holding information about the kernel lunch
//   fep - The kernel driver info structure
//   x, y, z, lod, face, a1, a2, a3, a4 - dimension offset information
...
void RsdCpuReferenceImpl::launchReduce(const Allocation *ain,
    ...
    using reduce_t = void (*)(uint8_t *, uint8_t *, uint32_t);
    reduce_t kernel = reinterpret_cast<reduce_t>(mtls->kernel);

    kernel(&mtls->inBuf[xStart], mtls->outBuf, xEnd - xStart);
}
