...
void* SharedLibraryUtils::loadSharedLibrary(const char *nativeLibDir, const char *cacheDir, const char *resName) {
    void *loaded = nullptr;
...
bool RsdCpuScriptImpl::init(char const *resName, char const *cacheDir,
    ...
    if (!is_force_recompile()) {
        mScriptSO = SharedLibraryUtils::loadSharedLibrary(nullptr, cacheDir, resName);
    }
    ...
    mScriptSO = SharedLibraryUtils::loadSharedLibrary(nativeLibDir, cacheDir, resName);
