...
void RsdCpuScriptImpl::forEachMtlsSetup(const Allocation * ain, Allocation * aout,
    ...
    if (ain != NULL && aout != NULL) {
        if (!ain->hasSameDims(aout)) {
            ALOGE("Failed to launch kernel; dimensions of input and output allocations do not match.");
            return;
        }
    }
    ...
    if (ains != NULL) {
        ...
        for (int Index = inLen; --Index >= 1;) {
            if (!ain0->hasSameDims(ains[Index])) {
                ALOGE("Failed to launch kernel; dimensions of input allocations do not match.");
                return;
            }
        }
