...
ScriptExecutable* ScriptExecutable::createFromSharedObject(
    ...
    if (strgets(line, MAXLINE, &rsInfo) == nullptr) {
#ifdef RS_COMPATIBILITY_LIB
        // .rs.info in old compat-lib shared objects may not have
        // pragmas embedded
        pragmaCount = 0;
#else
        return nullptr;
    ...
    else if (sscanf(line, PRAGMA_STR "%zu", &pragmaCount) != 1) {
...
void RsdCpuScriptImpl::populateScript(Script *script) {
    ...
    script->mHal.info.exportedPragmaKeyList = mScriptExec->getPragmaKeys().data();
    script->mHal.info.exportedPragmaValueList = mScriptExec->getPragmaValues().data();
