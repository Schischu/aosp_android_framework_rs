...
static void *loadSOHelper(const char *origName, const char *cacheDir,
    ...
    if (LoadedLibraries.find(origName) == LoadedLibraries.end()) {
        ...
        if (loaded) {
            ...
            ALOGE("dlopen returned NULL: %s", dlerror());
        }
        return loaded;
    }
...
#else
    scriptSOName.append("/cache/librs.");
#endif
...
#ifndef RS_COMPATIBILITY_LIB // RS_COMPATIBILITY_LIB is not defined
...
static bool is_skip_linkloader() {
    ...
    if(prop) {
        ALOGE("Skipping linkloader");
    }
    else {
        ALOGE("Default path: using linkloader");
    }
    return prop;
}
...
RsdCpuScriptImpl::~RsdCpuScriptImpl() {
    ...
    // TODO "deep free" mFieldIsObject as below
    if(is_skip_linkloader()) {
