...
const char* getCoreLibPath(Context* context, const char** coreLibRelaxedPath) {
    *coreLibRelaxedPath = nullptr;
...
void CpuScriptGroup2Impl::compile(const char* cacheDir) {
    ...
    const char* coreLibRelaxedPath;
    const char* coreLibPath = getCoreLibPath(getCpuRefImpl()->getContext(),
                                             &coreLibRelaxedPath);
