...
def run_test(state, name):
    ...
    # Close the log to allow the test to open and write to it.
    state.log_.close()
    ...
    if stdout:
        ...
        elif stdout.find(util_constants.TIMEOUT_TOKEN) >= 0:
            state.add_result(name, 'timeout')
            state.log_.writeln('timeout')
        else:
...
def signal_handler(_, _unused):
    ''' Signal handler for a SIGINT interrupt,
    caused by the user typing Ctrl-C'''
