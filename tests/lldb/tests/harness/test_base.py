...
class TestBase(object):
    ...
    def connect_to_platform(self, lldb_module, dbg, remote_pid):
        ...
        if err1.Fail():
            self.log_append(err1.GetCString())
            return False
        ...
        if err3.Fail() or not self.process_:
            self.log_append(err3.GetCString())
            return False
    ...
    def find_in_output(self, lldb, cmd_interp, cmd, strings_to_find):
        ...
        if not res.Succeeded():
            ...
            print ' The output was: {0}'.format(res.GetError())
            return False

        output = res.GetOutput()
        if not output:
            print 'Command {0} had no output.'.format(cmd)
            return False
        ...
        for string in strings_to_find:
            if not string in output:
                print 'Expected to find {0} in the output.'.format(string)
