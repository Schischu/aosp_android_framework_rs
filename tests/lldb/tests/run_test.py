...
from test_read_scalar import TestReadScalar
...
def start_timeout():
    ...
    global timeout
    timeout = Timer(util_constants.TIMEOUT, on_timeout)
    timeout.start()
    atexit.register(stop_timeout)
...
def quit(num):
    '''This function will safely quit the test suite, making sure the
    timeout thread is also killed'''
    stop_timeout()
    sys.stdout.flush()
    sys.exit(num)
