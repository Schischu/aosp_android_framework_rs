...
Running the test suite:
    ...
    The test suite can be run via the following command:
        ...
        > python run_tests.py [--config config_file.py] [--test test_path]

        optional arguments:
            --config, -c       Path to a custom config file.
            --test, -t         Path to a specific python test to run.

    To run an individual test, as opposed to the whole suite, an optional
    argument can be passed to the script stating the path to the python test.
...
A typical test transcript:
    ...
    pushing JavaFunction
    pushing JavaDebugWaitAttach
    pushing KernelLocalVariables
    Pushed all tests
    Pre run complete
    Found 9 tests
    running test_backtrace.py: pass
    running test_breakpoint_fileline_debug.py: pass
    running test_breakpoint_kernel_debug_1.py: pass
    running test_breakpoint_kernel_debug_2.py: pass
    running test_language.py: pass
    running test_language_subcmds_debug.py: pass
