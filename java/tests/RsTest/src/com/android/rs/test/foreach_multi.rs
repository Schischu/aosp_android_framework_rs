...
uint32_t *ain0, *ain1, *ain2;
uint16_t *ain3;

uint32_t *aout0, *aout1, *aout2;

uint32_t dimX;

static bool failed = false;

uint32_t RS_KERNEL zero_fill_uint32_alloc() {
  return 0;
}
...
static bool test_sum2_output() {
    ...
    for (i = 0; i < dimX; i++) {
        _RS_ASSERT(aout0[i] == (ain0[i] + ain1[i]));
    }
...
static bool test_sum3_output() {
    ...
    for (i = 0; i < dimX; i++) {
        _RS_ASSERT(aout1[i] == (ain0[i] + ain1[i] + ain2[i]));
    }
...
static bool test_sum_mixed_output() {
    ...
    for (i = 0; i < dimX; i++) {
        _RS_ASSERT(aout2[i] == (ain0[i] + ain1[i]));
    }
