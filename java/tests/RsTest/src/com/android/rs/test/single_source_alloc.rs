...
#define TEST_UNSUPPORTED_TYPE(dt) { \
    rs_allocation primitiveAlloc = rsCreatePrimitiveAllocation(dt, gDimX); \
    _RS_ASSERT(!rsIsObject(primitiveAlloc)); \
    rs_allocation vectorAlloc = rsCreateVectorAllocation(dt, 2, gDimX); \
    _RS_ASSERT(!rsIsObject(vectorAlloc)); \
    }

void TestInvalidUses() {
    // vector_width must be at least 2
    rs_allocation oneEltVector = rsCreateVectorAllocation(RS_TYPE_SIGNED_32, 1,
                                                          gDimX);
    _RS_ASSERT(!rsIsObject(oneEltVector));

    // vector_width must be at most 4
    rs_allocation largeVector = rsCreateVectorAllocation(RS_TYPE_SIGNED_32, 5,
                                                          gDimX);
    _RS_ASSERT(!rsIsObject(largeVector));

    // RS_TYPE_BOOLEAN vectors cannot be created
    rs_allocation boolVector = rsCreateVectorAllocation(RS_TYPE_BOOLEAN, 2,
                                                        gDimX);
    _RS_ASSERT(!rsIsObject(boolVector));

    TEST_UNSUPPORTED_TYPE(RS_TYPE_NONE);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_UNSIGNED_5_6_5);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_UNSIGNED_5_6_5);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_UNSIGNED_5_5_5_1);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_UNSIGNED_4_4_4_4);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_MATRIX_4X4);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_MATRIX_3X3);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_MATRIX_2X2);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_ELEMENT);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_TYPE);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_ALLOCATION);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_SAMPLER);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_SCRIPT);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_MESH);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_PROGRAM_FRAGMENT);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_PROGRAM_VERTEX);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_PROGRAM_RASTER);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_PROGRAM_STORE);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_FONT);
    TEST_UNSUPPORTED_TYPE(RS_TYPE_INVALID);
}
