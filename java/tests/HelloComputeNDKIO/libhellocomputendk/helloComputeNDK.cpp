...
Java_com_example_android_rs_hellocomputendkio_HelloComputeNDKIO_nativeMono(JNIEnv * env,
                                                                         jclass,
                                                                         jstring pathObj,
                                                                         jint X,
                                                                         jint Y,
                                                                         ...
                                                                         )
{
    ...
    sp<Allocation> inputIOAlloc = Allocation::createTyped(rs, t, RS_ALLOCATION_MIPMAP_NONE,
                                                          RS_ALLOCATION_USAGE_IO_INPUT | RS_ALLOCATION_USAGE_SCRIPT);
    sp<Allocation> outputIOAlloc = Allocation::createTyped(rs, t, RS_ALLOCATION_MIPMAP_NONE,
                                                           RS_ALLOCATION_USAGE_IO_OUTPUT | RS_ALLOCATION_USAGE_SCRIPT);

    sp<android::Surface> s = inputIOAlloc->getSurface();
    outputIOAlloc->setSurface(s);

    outputIOAlloc->copy2DRangeFrom(0, 0, X, Y, inputPtr);
    outputIOAlloc->ioSendOutput();
    inputIOAlloc->ioGetInput();

    ScriptC_mono* sc = new ScriptC_mono(rs);
    sc->forEach_root(inputIOAlloc, outputAlloc);
