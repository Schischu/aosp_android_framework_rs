...
public class UT_script_group2_pointwise extends UnitTest {
    ...
    public void run() {
        final int M_API_LEVEL = 23;
        //Create a RS context with targetAPI = M;
        RenderScript pRS = RenderScript.create(mCtx, M_API_LEVEL);

        ScriptC_increment s_inc = new ScriptC_increment(pRS);
        ScriptC_doublle s_doublle = new ScriptC_doublle(pRS);
        ...
        ScriptGroup2.Closure c1 =
                builder.addKernel(s_doublle.getKernelID_doublleKernel(),
                                  Type.createX(pRS, Element.I32_4(pRS), ARRAY_SIZE),
