...
public class GoogLeNet extends TestBase {
    ...
    GoogLeNet(boolean useBNNM) {
        this.useBNNM = useBNNM;
    }
    ...
    private void setTest() {
        ...
        for (int i=0; i<googlenet_gemm_sizes.length; i+=3) {
            if (useBNNM) {
                ...
                int m = roundUp(googlenet_gemm_sizes[i], 16);
                int n = roundUp(googlenet_gemm_sizes[i+1], 32);
                int k = roundUp(googlenet_gemm_sizes[i+2], 16);
                e = Element.F32(mRS);
                Type.Builder builder = new Type.Builder(mRS, e);
                Type a_type = builder.setX(k).setY(m).create();
    ...
    public String getTestInfo() {
        return "SGEMM GoogLeNetTest: " + googlenet_gemm_sizes.length / 3;
    }
}
