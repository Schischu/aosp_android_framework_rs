...
void setRadius_half(int rad) {
    ...
    // Compute gaussian weights for the blur
    // e is the euler's number
    half e = (half) 2.718281828459045f;
    half pi = (half) 3.1415926535897932f;
    // g(x) = ( 1 / sqrt( 2 * pi ) * sigma) * e ^ ( -x^2 / 2 * sigma^2 )
    // x is of the form [-radius .. 0 .. radius]
    // and sigma varies with radius.
    // Based on some experimental radius values and sigma's
    ...
    // will resemble a box blur since with large sigma
    // the gaussian curve begins to lose its shape
    half sigma = (half) 0.4f * (half)radius + (half) 0.6f;

    // Now compute the coefficints
    // We will store some redundant values to save some math during
    // the blur calculations
    // precompute some values
    half coeff1 = (half) 1.0f / (sqrt( 2.0f * pi ) * sigma);
    half coeff2 = (half) - 1.0f / ((half) 2.0f * sigma * sigma);
...
static float3 convert_half_to_float3(half3 in) {
    float3 out;
    out.x = (float) in.x;
    out.y = (float) in.y;
    out.z = (float) in.z;
...
uchar4 RS_KERNEL vert_half(uint32_t x, uint32_t y) {
    ...
    float3 bp_float3 = convert_half_to_float3(blurredPixel);
    out.xyz = convert_uchar3(clamp(bp_float3, 0.f, 255.f));
    out.w = 0xff;
    return out;
}
