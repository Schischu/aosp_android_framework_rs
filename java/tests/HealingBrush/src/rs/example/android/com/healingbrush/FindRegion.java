...
public class FindRegion {
    ...
    public Rect findMatch(ScriptC_find_region findRegion, RenderScript mRs, Bitmap image) {
        ...
        findRegion.invoke_findMin(fit, fit_max, noSearch_x, noSearch_y, mRoiBounds.width(), mRoiBounds.height());
        int[] mina = new int[2];
        fit_max.copyTo(mina);
    ...
    private static Rect getBoundingRect(float[] xy) {
        ...
        int mWidth = (((int) (8 + mRect.width())) & ~3); // bounding rectangle that is a power of 8 big
