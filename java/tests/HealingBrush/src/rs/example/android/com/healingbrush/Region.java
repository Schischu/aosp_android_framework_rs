...
public class Region {
    ...
    Drawable getSourceLocation() {
        ...
        for (int i = 0; i < mPointsXY.length; i += 2) {
            if (i == 0) {
                path.moveTo(mPointsXY[i] - bounds.left + mCutOffsetX, mPointsXY[i + 1] - bounds.top + mCutOffsetY);
            } else {
                path.lineTo(mPointsXY[i] - bounds.left + mCutOffsetX, mPointsXY[i + 1] - bounds.top + mCutOffsetY);
            }
        }
        path.close();
