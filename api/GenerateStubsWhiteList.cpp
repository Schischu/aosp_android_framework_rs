 ...
 * not currently mangled more complicated types like function pointers, name spaces,
...
static bool mangleType(string vectorSize, list<string>* tokens, vector<string>* previousManglings,
    ...
    bool isTerminal = false;      // True if this iteration parse a terminal node in the production.
    bool canBeCompressed = true;  // Will be falsed for manglings of built ins.
...
// Write to the stream the mangled representation of each paramter.
...
static bool addManglingsForSpecification(const Function& function,
    ...
    /* We track success rather than aborting early in case of failure to that we
...
static const string addVariable(GeneratedFile* file, int* variableNumber) {
    const string name = "buf" + to_string((*variableNumber)++);
    /* Some data structure like rs_tm can't be exported.  We'll just use a dumb buffer
 ...
 */
static void generateTestCall(GeneratedFile* file, ostringstream* calls, int* variableNumber,
                             const Function& function, const FunctionPermutation& permutation) {
    *calls << "    ";
...
static bool generateApiTesterFile(const string& slangTestDirectory, int apiLevel) {
     ...
     */
    ostringstream calls;
    int variableNumber = 0;  // Used to generate unique names.
    for (auto f : systemSpecification.getFunctions()) {
        const Function* function = f.second;
        for (auto spec : function->getSpecifications()) {
