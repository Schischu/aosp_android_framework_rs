...
description:
  ...
  to a single output Allocation (the last argument). Note the output Allcoation is
  ...
  the work is done for all elements of the input Allocations (and all results have
...
arg: rs_allocation input
arg: rs_allocation output
summary: (Interanl API) Launch a kernel in the current Script (with the slot number)
description:
test: none
end:
